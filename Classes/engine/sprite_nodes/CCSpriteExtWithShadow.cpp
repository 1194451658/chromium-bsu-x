...
#include "CCSpriteExtWithShadow.h"

CCSpriteExtWithShadow* CCSpriteExtWithShadow::create(const char *pszFileName)
{
	CCSpriteExtWithShadow *pobSprite = new CCSpriteExtWithShadow();
...
void CCSpriteExtWithShadow::drawShadow()
{
	...
	// draw shadow first
	CC_PROFILER_START_CATEGORY(kCCProfilerCategorySprite, "CCSpriteExt - draw");
	CCAssert(!m_pobBatchNode, "If CCSpriteExt is being rendered by CCSpriteExtBatchNode, CCSpriteExt#draw SHOULD NOT be called");
	//CC_NODE_DRAW_SETUP();
...
#define kQuadSize sizeof(m_sQuad.bl)
	...
	CC_PROFILER_STOP_CATEGORY(kCCProfilerCategorySprite, "CCSpriteExt - draw");
}

void CCSpriteExtWithShadow::visitShadow()
{
	...
	// CCNode* pNode = NULL;
	CCSpriteExtWithShadow* spriteWithShadow = NULL;
	...
	if(m_pChildren && m_pChildren->count() > 0)
	{
		...
		{
			spriteWithShadow = dynamic_cast<CCSpriteExtWithShadow*>(arrayData->arr[i]);
